...
2. 如果文本中提到"轻仓"、"小仓"，设置为 0.2
3. 如果文本中提到"半仓"，设置为 0.5
4. 如果文本中提到"重仓"、"满仓"，设置为 0.8
5. 如果没有提到仓位信息：
   - 对于买入操作，默认设置为 0.1（10%仓位）
   - 对于卖出操作，默认设置为 0.5（50%仓位）
...

{
    ...
    "action": "buy或sell（字符串）",
    "position_ratio": 0.1,  # 必须返回，范围0-1，表示仓位比例。买入默认0.1，卖出默认0.5
    "price_min": 10.5,
    "price_max": 11.0,
...
注意：position_ratio 字段必须返回，且必须是 0-1 之间的数值。"""
